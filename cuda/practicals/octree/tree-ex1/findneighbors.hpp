...
void depthFirstTraversal(const TreeNodeIndex* childOffsets, C&& continuationCriterion, A&& endpointAction)
{
    ...
    // do
    // {
    //    evaluate continuationCriterion(child) for all children of currentNode 
    //    {
    //       if the continuation is true:
    //         - if the child is a leaf, call endpointAction(child)
    //         - else push child on stack
    //    } 
    //    
    //    update currentNode by popping an entry from the stack
    //
...
unsigned findNeighbors(LocalIndex i, const T* x, const T* y, const T* z, const T* h,
